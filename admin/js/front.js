...
DesignModal.prototype.initShow = function (option) {
    let innerPublish = '';
	let objThis = this;
	let identify_text = option.identify_text !== undefined ? option.identify_text : "확인";
	let cancel_text = option.cancel_text !== undefined ? option.cancel_text : "취소";
	...
	if (option.type === "confirm") {
        innerPublish += "          <a href='javascript:;' class='btn_dmsm close_dmtrigger btn_dmsmcancel'>"+cancel_text+"</a>";
        innerPublish += "          <a href='javascript:;' class='btn_dmsm close_dmtrigger btn_dmsmidentify dtype2'>"+identify_text+"</a>";
	}else{
        innerPublish += "          <a href='javascript:;' class='btn_dmsm close_dmtrigger btn_dmsmidentify'>"+identify_text+"</a>";
    }
	innerPublish += "    </div>";
    innerPublish += "  </div>";
    innerPublish += "</div>";
...
function scrollTab(target){
    let html_obj = document.querySelector("html");
	let anchor_ui_wrap = document.querySelector(target);
	let anchor_tab_zone = anchor_ui_wrap.querySelector(".anchor_tab_zone");
	let anchor_tab_layer = anchor_ui_wrap.querySelector(".anchor_tab_layer");
	let anchor_tab_wrap = anchor_ui_wrap.querySelector(".anchor_tab_wrap");
	let anchor_tab = anchor_ui_wrap.querySelectorAll(".anchor_tab");
	let anchor_content = anchor_ui_wrap.querySelectorAll(".anchor_content");
    let anchor_tab_zone_pos = 0;
    let anchor_tab_zone_height = 0;
    let window_wid = 0;

    html_obj.classList.add("scroll_mode");
	posUpdate();
    scrollAction();

    window.addEventListener('resize', function(){
        if(!window_wid == window.innerWidth){
            posUpdate();
        }
    ...
    function scrollAction(){
       let scroll_value = window.pageYOffset;
        if(scroll_value > anchor_tab_zone_pos){
            anchor_tab_layer.classList.add("fixed");
        }else{
            anchor_tab_layer.classList.remove("fixed");
        }
    }

    // anchor_tab.forEach(function(elem,index){
    
    // });
    anchor_tab.forEach( (elem,index) => {
        elem.addEventListener("click", (e) => {
            e.preventDefault();
            let this_obj = e.currentTarget;
            let this_not_obj = siblings(this_obj);
            let this_href = e.currentTarget.getAttribute("href");
            let this_target = document.querySelector(this_href);

            this_not_obj.forEach( (elem,index) => {
                elem.classList.remove("active");
            });
            this_obj.classList.add("active");

            if(this_target !== undefined){
                // console.log(this_target.offsetTop , anchor_tab_zone_height , this_target.offsetTop - anchor_tab_zone_height - 30);
                window.scrollTo(0 , this_target.offsetTop - anchor_tab_zone_height - 20);
            }
        });
    });
	...
	function posUpdate(){
		...
		anchor_tab_zone.style.minHeight = "auto";
        anchor_tab_zone.style.minHeight = anchor_tab_zone.children[0].offsetHeight +"px";


        anchor_tab_zone_pos = anchor_tab_zone.offsetTop || 0;
        anchor_tab_zone_height = anchor_tab_zone.offsetHeight || 0;

        // console.log(anchor_tab_zone_pos , anchor_tab_zone_height);
	}
}